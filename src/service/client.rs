...
mod tests {
    ...
    fn info(
        explicit_task_budgets: bool,
        sanitized_task_metrics: bool,
        recoverable_maintenance: bool,
        explicit_task_compaction: bool,
    ) -> ServiceInfo {
        ServiceInfo {
            protocol_version: SERVICE_PROTOCOL_VERSION,
    ...
    fn negotiation_requires_the_explicit_task_budget_capability() {
        validate_info(&info(true, true, true, true)).expect("advertised capabilities are accepted");
        assert_eq!(
            validate_info(&info(false, true, true, true))
    ...
    fn negotiation_requires_sanitized_task_metrics() {
        assert_eq!(
            validate_info(&info(true, false, true, true))
    ...
    #[test]
    fn negotiation_requires_recoverable_maintenance() {
        assert_eq!(
            validate_info(&info(true, true, false, true))
                .expect_err("false maintenance support must fail negotiation")
                .code(),
            ServiceClientErrorCode::InvalidResponse
        );
    ...
    #[test]
    fn negotiation_requires_explicit_task_compaction() {
        assert_eq!(
            validate_info(&info(true, true, true, false))
                .expect_err("false compaction support must fail negotiation")
                .code(),
            ServiceClientErrorCode::InvalidResponse
        );
