...
uv run --offline --project "$BENCHMARK_ROOT" --locked python -c \
  'import json,sys; value=json.load(open(sys.argv[1], encoding="utf-8")); assert value["passed_trials"] == 3; assert value["valid_trials"] == 3; assert value["invalid_trials"] == 0; assert value["pass_rate"] == 1.0' \
  "$RESULT_PATH"
